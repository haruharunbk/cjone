...
var $devWidth; //디바이스 크기
var $limitsize = 768; //디바이스 제한 크기

$(document).ready(function(){
	//디바이스 크기는 body의 크기로 한다. 
	$devWidth = $("body").width();
	console.log($devWidth);

	//window.resize를 했을때 디바이스 크기는 바디의 크기로 한다. 
	...
	$(".gnb > ul > li > a").bind("mouseover focus",function(){
		...
		// 고객센터가 열려있다면 클래스 on이 되어 있다면 클래스를 제거해주세요
		if( $("dl.topMenu > dd").eq(4).hasClass("on") ){
			$("dl.topMenu > dd").eq(4).removeClass("on")
		}
	});

	$(".gnb").bind("mouseleave blur", function(){ //큰 범위를 벗어나면 mouseleave되기
		//단, 모바일에서는 해당되지 않는 사항이다.
		if( $devWidth < $limitsize ) return false;

		// header_wrap은 height값 다시 원래 크기로 맞춰주고 애니메이트로 하고
		// 하위 ul은 다시 display none으로 안보이게 한다. 
